    ...
    <div class="news-form-page">
        <div v-if="loadingUser || !hasPermission" class="permission-check">
            ...
            <div v-else class="permission-denied">
                ...
                <h3>Khong the truy cap Chinh sua bai viet</h3>
                <p>Chi Superadmin, Admin, Manager va Editor moi co the chinh sua tin tuc.</p>
        ...
        <div v-else class="form-wrapper">
            <div class="page-header">
                <div>
                    <h1><i class="fas fa-edit"></i> Chinh sua bai viet</h1>
                    <p>Cap nhat thong tin bai viet #{{ newsId }}</p>
                </div>
                <div class="header-actions">
                    <NuxtLink :to="`/admin/news/view/${newsId}`" class="btn btn-secondary">
            ...
            <form v-else class="news-form" @submit.prevent="handleUpdateNews">
                <div class="form-grid">
                    <div class="form-group full">
                        <label>Tieu de <span class="required">*</span></label>
                        <input v-model.trim="form.title" @input="clearFieldError('title')" @blur="handleTitleBlur" :class="['form-control', { 'is-invalid': !!formErrors.title }]" type="text" required>
                    ...
                    <div class="form-group">
                        ...
                        <select v-model="form.status" @change="handleStatusChange" :class="['form-control', { 'is-invalid': !!formErrors.status }]" :disabled="!canManageStatus">
                    ...
                    <div class="form-group full">
                        ...
                        <textarea v-model.trim="form.content" @input="clearFieldError('content')" @blur="validateField('content')" :class="['form-control', { 'is-invalid': !!formErrors.content }]" rows="12" required></textarea>
                    ...
                    <div class="form-group">
                        ...
                        <input v-model.trim="form.meta_title" @input="clearFieldError('meta_title')" @blur="validateField('meta_title')" :class="['form-control', { 'is-invalid': !!formErrors.meta_title }]" type="text">
                        <p v-if="formErrors.meta_title" class="field-error">{{ formErrors.meta_title }}</p>
                    </div>

                    <div class="form-group full">
                        <label>Meta description</label>
                        <textarea v-model.trim="form.meta_description" @input="clearFieldError('meta_description')" @blur="validateField('meta_description')" :class="['form-control', { 'is-invalid': !!formErrors.meta_description }]" rows="3"></textarea>
                ...
                <div class="form-actions">
                    ...
                    <button class="btn btn-primary" type="submit" :disabled="saving">
                        ...
                        {{ saving ? 'Dang cap nhat...' : 'Cap nhat bai viet' }}
                    </button>
                </div>
            </form>
        </div>
...
useHead({
    title: 'Chinh sua bai viet - Admin'
})
...
const fetchNewsDetail = async () => {
    ...
    try {
        ...
        error.value = err.message || 'Khong the tai bai viet'
    } finally {
        loading.value = false
    }
}
...
const handleUpdateNews = async () => {
    ...
    try {
        ...
        showSuccess(data?.message || 'Cap nhat bai viet thanh cong')
        await navigateTo('/admin/news')
    } catch (err) {
        showError(err.message || 'Khong the cap nhat bai viet')
    } finally {
        saving.value = false
    }
}
...
onMounted(async () => {
    ...
    if (hasPermission.value) {
        await Promise.all([fetchCategories(), fetchNewsDetail()])
    }
})
</script>
