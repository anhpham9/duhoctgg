    ...
    <div class="school-form-page">
        <div v-if="loadingUser || !hasPermission" class="permission-check">
            ...
            <div v-else class="permission-denied">
                ...
                <h3>Không thể truy cập Tạo trường học</h3>
                <p>Chỉ Superadmin, Admin và Manager mới có thể tạo trường học.</p>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select v-model="form.region_id" @change="clearFieldError('region_id')"
                            ...
                            <option v-for="region in regions" :key="region.id" :value="Number(region.id)">{{ region.name
                            }}</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select v-model="form.type_id" @change="clearFieldError('type_id')"
                            ...
                            <option v-for="type in schoolTypes" :key="type.id" :value="Number(type.id)">{{ type.name }}
                            </option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input v-model.trim="form.phone" @input="clearFieldError('phone')"
                    ...
                    <div class="form-group ">
                        <label>Email</label>
                        <input v-model.trim="form.email" @input="clearFieldError('email')"
                    ...
                    <div class="form-group ">
                        <label>Website</label>
                        <input v-model.trim="form.website" @input="clearFieldError('website')"
                    ...
                    <div class="form-group">
                        ...
                        <input v-model.number="form.class_size" @input="clearFieldError('class_size')"
                            ...
                            :class="['form-control', { 'is-invalid': !!formErrors.class_size }]" type="number" min="1"
                            max="200">
                        ...
                        <div class="form-split">
                            <div class="form-col">
                                ...
                                <div v-else class="upload-inline-actions">
                                    ...
                                    <p v-if="formErrors.logo_url" class="field-error">{{
                                        formErrors.logo_url
                                        }}
                                    </p>
                                </div>
                                ...
                                <div class="image-preview-card">
                                    ...
                                    <div class="image-preview-surface logo-preview-surface">
                                        <img v-if="logoPreviewSrc" :src="logoPreviewSrc" alt="Logo preview"
                                            class="image-preview image-preview-logo">
                        ...
                        <div class="form-split">
                            <div class="form-col">
                                ...
                                <div class="input-mode-switch" role="group" aria-label="Thumbnail input mode">
                                    <button type="button" class="mode-btn"
                                        :class="{ active: thumbnailInputMode === 'url' }"
                                        @click="setImageInputMode('thumbnail', 'url')">
                                        Nhập link ảnh
                                    </button>
                                    <button type="button" class="mode-btn"
                                        :class="{ active: thumbnailInputMode === 'upload' }"
                                        @click="setImageInputMode('thumbnail', 'upload')">
                                        Upload lên Cloudinary
                                    </button>
                                </div>
                                <input v-if="thumbnailInputMode === 'url'"
                                    v-model.trim="form.thumbnail_url"
                                    @input="clearFieldError('thumbnail_url')" type="url" class="form-control"
                                    :class="{ 'is-invalid': !!formErrors.thumbnail_url }"
                                    placeholder="https://example.com/thumbnail.png">
                                <div v-else class="upload-inline-actions">
                                    <input ref="thumbnailFileInput" type="file"
                                        accept="image/png,image/jpeg,image/webp,image/gif"
                                        class="hidden-file-input" @change="onThumbnailFileChange">
                                    <button class="btn btn-secondary btn-upload-inline" type="button"
                                        :disabled="thumbnailUploading || saving || isLoading"
                                        @click="triggerThumbnailPicker">
                                        <i class="fas"
                                            :class="thumbnailUploading ? 'fa-spinner fa-spin' : 'fa-cloud-upload-alt'"></i>
                                        {{ thumbnailUploading ? 'Đang upload thumbnail...' : 'Upload thumbnail' }}
                                    </button>
                                    <span class="upload-inline-hint">PNG/JPG/WEBP/GIF, tối đa 1MB. Ảnh chỉ upload khi bấm Lưu cài đặt.</span>
                                    <p v-if="formErrors.thumbnail_url" class="field-error">{{
                            ...
                            <div class="form-col">
                                <div class="image-preview-card">
                                    ...
                                    <div class="image-preview-surface thumbnail-preview-surface">
                                        <img v-if="thumbnailPreviewSrc" :src="thumbnailPreviewSrc" alt="Thumbnail preview"
                                            class="image-preview image-preview-thumbnail">
                    ...
                    <!-- <div class="form-group">
                        ...
                        <input v-model.trim="form.logo_url" @input="clearFieldError('logo_url')"
                            @blur="validateField('logo_url')"
                            :class="['form-control', { 'is-invalid': !!formErrors.logo_url }]" type="url"
                            placeholder="https://...">
                    ...
                    <!-- <div class="form-group full">
                        ...
                        <input v-model.trim="form.thumbnail_url" @input="clearFieldError('thumbnail_url')"
                            @blur="validateField('thumbnail_url')"
                            :class="['form-control', { 'is-invalid': !!formErrors.thumbnail_url }]" type="url"
                            placeholder="https://...">
                        ...
                        <div v-if="featureItems.length" class="feature-list form-split">
                            <div v-for="(feature, index) in featureItems" :key="index" class="feature-item form-col">
                                <input v-model.trim="featureItems[index]" @input="clearFieldError('features')"
                                    ...
                                    type="text" placeholder="Nhap feature tag">
                                <button type="button" class="btn-icon-danger" @click="removeFeature(index)"
                                    aria-label="Xoa feature">
...
const onLogoFileChange = (event) => {
    ...
    clearFieldError('logo_url')
    showInfo('Logo đã được chọn. Ảnh sẽ chỉ upload khi bạn bấm Tạo trường học.')
}
...
const onThumbnailFileChange = (event) => {
    ...
    if (!allowedTypes.includes(fileType)) {
        showError('Định dạng file thumbnail không hợp lệ (chỉ nhận PNG/JPG/WEBP/GIF)')
        return
    }

    if (file.size > 1 * 1024 * 1024) {
        showError('File thumbnail vượt quá 1MB')
        return
    }
    ...
    clearFieldError('thumbnail_url')
    showInfo('Thumbnail đã được chọn. Ảnh sẽ chỉ upload khi bạn bấm Tạo trường học.')
}
...
onMounted(async () => {
    ...
    if (hasPermission.value) {
        await Promise.all([fetchRegions(), fetchSchoolTypes()])
    }
})
